var getCurrentPage=function(path){
	...
	var items = [];
	//var root = SingleItems.find({_id:"root"});
	//var root = SingleItems.find({_id:"root"}).fetch()[0];
	//console.log(root);
	//root.fullUrl = "/";
	//items.push(root);
	var par=null;
  ...
  for(var i=0;i<res.pathParts.length;i++){
		item=SingleItems.find({"relations._id":par,"relations.url":res.pathParts[i],"relations.type":"parent"});
		if(item.count()>0){
			item = item.fetch()[0];
			items.push(item);
...
if(Meteor.isServer){
	Meteor.publish("single-pages",function(path){
		...
		if(ids.length>0){
			return SingleItems.find({$or:ids});
		}else{
			return null;
		}
	});
